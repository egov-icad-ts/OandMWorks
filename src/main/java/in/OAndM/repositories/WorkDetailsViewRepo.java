...
public interface WorkDetailsViewRepo extends BaseRepository<WorkDetailsViewEntity, Integer> {
	...
	@Query("select new in.OAndM.DTO.WorkDetailsViewModel(aa.approvedById as approvedId, aa.approvedByName as approvedName, " +
		       ...
		       "where (" +
		       "(:type = 'ce' and aa.unitId = :unitId) or " +
		       "(:type = 'se' and aa.unitId = :unitId and aa.circleId = :circleId) or " +
		       "(:type = 'ee' and aa.unitId = :unitId and aa.circleId = :circleId and aa.divisionId = :divisionId) or " +
		       "(:type = 'dee' and aa.unitId = :unitId and aa.circleId = :circleId and aa.divisionId = :divisionId and aa.subDivisionId = :SubDivisionId) or " +
		       "(:type = 'enc')" +
		       ") and aa.financialYear = :finyear group by aa.approvedById, aa.approvedByName")
		public List<WorkDetailsViewModel> getAbsRepSanctionAuthorityWiseByFinyear(Integer finyear, Integer unitId,  Integer circleId, Integer divisionId, Integer SubDivisionId, String type);
	...
	public List<WorkDetailsViewModel> findworkDetailsViewEntityByFinancialYearAndUnitIdAndProjectId(Integer financialYear,Integer unitId, Integer type, Integer projectId);
	...
	@Query("select  new in.OAndM.DTO.WorkDetailsViewModel(aa.unitId as unitId,aa.unitName as unitName,aa.projectId as projectId, aa.projectName as projectName,"
			+ "count(distinct(aa.workId)) as adminCount, COALESCE(sum(aa.adminApprovedAmountLakh),0) as adminAmt,"
			+ "count(distinct(aa.govtSanction)) as govtSanction, COALESCE(SUM(aa.govtSancAmount / 100000), 0) as govtSancAmount,"
			+ " count(distinct(aa.omCommiteeSanction)) as omCommiteeSanction, COALESCE(sum(aa.committeeSancAmount/ 100000),0) as committeeSancAmount,"
			+ " count(distinct(aa.go45Sanction)) as go45Sanction, COALESCE(sum(aa.go45SancAmount/ 100000),0) as go45SancAmount,"
			...
			+ "sum(wd.billsPaid) as billsPaid, COALESCE(sum(wd.paidAmountLakh),0) as paidAmount, "
			+ "sum((wd.billsPending)) as billsPending, COALESCE(sum(wd.pendingAmountLakh),0) as pendingAmount) "
			+ " from AdminSanctionViewEntity  aa left join  WorkDetailsViewEntity wd on aa.workId =wd.techWorkId where   aa.financialYear=:finyear and aa.projectId=:projectId "
			+ " group by aa.unitId,aa.unitName,aa.projectId,aa.projectName")
	public List<WorkDetailsViewModel> getAbsReportProjectUnitWise(Integer finyear, Integer projectId);
